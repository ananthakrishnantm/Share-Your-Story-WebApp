import React, { useState } from "react";
import Upload from "./Upload";
import AllUserBlogs from "./AllUserBlogs";
import Navbar from "./Navbar";
import SidePanel from "./UserComponents/SidePanel";
import RightSidePanel from "./UserComponents/RightSidePanel";
import UserBlogs from "./UserBlogs";
import BottomNavBar from "./BottomNavBar";

function HomeTwo() {
  const [triggerFetch, setTriggerFetch] = useState(false);

  const updateBlogList = () => {
    setTriggerFetch(!triggerFetch);
  };

  return (
    <div>
      {/* this is the myblog section*/}
      {/*the side panel containes the profile*/}
      <div className="flex justify-center">
        <div className="hidden sm:hidden md:block">
          <SidePanel />
        </div>
        <div>{/* <ViewProfileSecondary /> */}</div>
        <div className="flex justify-center  flex-1 ml-2 mr-2  mt-28 sm:mt-46 md:mt-28  lg:mt-24 sm:ml-5  sm:mr-5 md:ml-64 lg:mr-64">
          <UserBlogs />
        </div>
        <div className="hidden md:hidden lg:block">
          <RightSidePanel />
        </div>
        <div className="flex flex-col ">
          <div>{/* <SideBar /> */}</div>
          <div className="mt-10" style={{ marginTop: "20%" }}>
            {/* <FollowingList /> */}
          </div>
        </div>
      </div>

      <Navbar />
      <span className="md:hidden  ">
        <BottomNavBar />
      </span>
    </div>
  );
}

export default HomeTwo;
